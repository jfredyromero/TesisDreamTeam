    ...
    for i = 1:length(audioCell) % Audios
        ...
        for j = 1:n % Niveles de descomposición
            for k = q % Niveles de cuantificación
                % [processedSignal, bitsUsados, bitsDesperdiciados] = algoritmoLiftingEnergiasByCoefFunction(audioCell{i, 2}, j, k, cama, lsc{f});
                [processedSignal, bitsUsados, bitsDesperdiciados] = algoritmoLiftingEnergiasByNivelFunction(audioCell{i, 2}, j, k, cama, lsc{f});
