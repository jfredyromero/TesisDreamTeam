...
for i = 1:numTramas
    ...
    while sum(porcentajesIniciales == mejoresPorcentajes) ~= (n + 1)
        ...
        bestSignal(((i - 1) * tramaSamples) + 1:tramaSamples * i) = bestTrama;
...
pesq = ((medirPESQ(xn(1:length(bestSignal)), bestSignal')) + 0.5) / 5;
nmse = medirNMSE(xn(1:length(bestSignal)), bestSignal');
